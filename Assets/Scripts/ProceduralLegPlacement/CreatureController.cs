...
public class CreatureController : MonoBehaviour {
    ...
    void Update () {
        ...
        if (dynamicGait) {
            timeBetweenSteps = maxTargetDistance / Mathf.Max (mSpeed * velocity.magnitude, Mathf.Abs (_rSpeed * Mathf.Deg2Rad * averageRotationRadius));
        }
        ...
        if (Time.time > lastStep + (timeBetweenSteps / legs.Length) && legs != null) {
            ...
            Vector3 rotationalPoint = ((Quaternion.Euler (0, _rSpeed / 2f, 0) * legDirection) + transform.position) - legPoint;
